# caller_nonleaf.s — calls mul_times_add(a0=5, a1=7, a2=3) -> 5*7 + 3 = 38
...
# a0 = a0*a1 + a2   (non-leaf: calls mul_u32_leaf)
    .globl mul_times_add
mul_times_add:
    ...
    # call multiplier
    mv   a0, a0               # (already in a0)
    mv   a1, a1               # (already in a1)
    jal  ra, mul_u32_leaf     # returns product in a0

    add  a0, a0, s0           # add the saved addend
...
# leaf helper from above (no stack - repeated addition)
    .globl mul_u32_leaf
mul_u32_leaf:
    mv   t0, a0
    mv   t1, a1
